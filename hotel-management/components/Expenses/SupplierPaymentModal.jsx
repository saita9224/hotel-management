...
export default function SupplierPaymentModal({
  ...
  onClose,
  supplierName,
  unpaidExpenses, // already sorted oldest first, only unpaid ones
}) {
  ...
  const totalOutstanding = useMemo(() => {
    return unpaidExpenses.reduce((sum, e) => sum + Number(e.balance), 0);
  }, [unpaidExpenses]);

  // Preview: show how the entered amount will be split
  ...
  const handleSubmit = async () => {
    ...
    if (value > totalOutstanding)
      return Alert.alert(
        "Exceeds Balance",
        `Maximum payable is KES ${totalOutstanding.toFixed(2)}.`,
      );

    try {
      setSubmitting(true);

      // Fire payments sequentially, oldest first
      for (const split of splitPreview) {
        if (split.toPay > 0) {
          await payBalanceService(split.id, split.toPay);
        }
      }
        ...
        >
          ...
          >
            ...
            <Text style={{ color: colors.tabBarInactive, marginTop: 4 }}>
              ...
              {unpaidExpenses.length === 1 ? "expense" : "expenses"}
            </Text>
            <Text style={{ color: "#FF3B30", fontWeight: "700", marginTop: 4 }}>
          ...
          {splitPreview.length > 0 && (
            <>
              <Text
                style={[
                  styles.label,
                  { color: colors.tabBarInactive, marginTop: 14 },
                ]}
              >
                Payment breakdown
              </Text>
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      ...
                      <Text
                        style={{ color: colors.tabBarInactive, fontSize: 12 }}
                      >
              ...
              {(() => {
                const totalSplit = splitPreview.reduce(
                  (s, x) => s + x.toPay,
                  0,
                );
                const leftover = parseFloat(
                  (Number(amount) - totalSplit).toFixed(2),
                );
                if (leftover > 0) {
                  return (
                    <Text
                      style={{ color: "#FF3B30", marginTop: 8, fontSize: 13 }}
                    >
