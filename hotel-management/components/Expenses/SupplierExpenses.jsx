...
export default function SupplierExpenses() {
  ...
  const suppliers = useMemo(() => {
    ...
    expenses.forEach((e) => {
      ...
      if (!map[key]) {
        map[key] = { supplier_id: e.supplier_id, supplier_name: e.supplier_name || "Unknown Supplier", expenses: [], total: 0, paid: 0, balance: 0 };
      }
  ...
  const currentSupplier = useMemo(() => {
    ...
    return suppliers.find((s) => s.supplier_id === selectedSupplier || s.supplier_name === selectedSupplier) || null;
  }, [selectedSupplier, suppliers]);

  const supplierExpenses = useMemo(() => {
    if (!currentSupplier) return [];
    return [...currentSupplier.expenses].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
  }, [currentSupplier]);

  const unpaidExpenses = useMemo(() => supplierExpenses.filter((e) => !e.is_fully_paid), [supplierExpenses]);
  ...
  const renderSupplier = ({ item }) => (
    ...
    >
      <Text style={{ color: colors.text, fontWeight: "700", fontSize: 16 }}>  {/* 👈 removed fontFamily */}
        {item.supplier_name}
      </Text>
  ...
  const renderExpense = ({ item }) => {
    ...
    return (
      <View style={[styles.expenseRow, { borderBottomColor: colors.border, backgroundColor: isNextToPay ? colors.accent + "18" : "transparent" }]}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ color: colors.tabBarInactive, fontSize: 12, marginTop: 2 }}>
            ...
            {isNextToPay ? <Text style={{ color: colors.accent }}> ← next to pay</Text> : null}
          </Text>
        </View>
        <Text style={{ color: colors.tabBarInactive, width: 60, textAlign: "right" }}>×{item.quantity}</Text>
        <Text style={{ color: colors.text, width: 90, textAlign: "right" }}>KES {item.total_price.toFixed(2)}</Text>
        <Text style={{ width: 90, textAlign: "right", fontWeight: "700", color: item.is_fully_paid ? "#30D158" : "#FF453A" }}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      {!selectedSupplier && (
        <>
          <Text style={[styles.title, { color: colors.text }]}>Supplier Ledger</Text>  {/* 👈 removed fontFamily */}
          <FlatList
            data={suppliers}
            ...
            ListEmptyComponent={
              <Text style={{ color: colors.tabBarInactive, textAlign: "center", marginTop: 40 }}>No suppliers found</Text>
            }
          />
        </>
      )}
      ...
      {selectedSupplier && currentSupplier && (
        <>
          <TouchableOpacity style={styles.backBtn} onPress={() => setSelectedSupplier(null)}>
            <Text style={{ color: colors.accent, fontWeight: "600" }}>← Back to suppliers</Text>
          </TouchableOpacity>

          <View style={[styles.supplierHeader, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: colors.text, fontWeight: "700", fontSize: 17 }}>{currentSupplier.supplier_name}</Text>
            ...
            <TouchableOpacity
              style={[styles.paySupplierBtn, { backgroundColor: hasOutstanding ? colors.accent : colors.border }]}
              onPress={() => {
                if (!hasOutstanding) return Alert.alert("All Paid", "This supplier has no outstanding balance.");
          ...
          <View style={[styles.colHeaders, { borderBottomColor: colors.border }]}>
            {["ITEM", "QTY", "TOTAL", "BALANCE"].map((h, i) => (
              <Text key={h} style={{ color: colors.tabBarInactive, fontWeight: "600", fontSize: 12, flex: i === 0 ? 1 : undefined, width: i === 0 ? undefined : 90, textAlign: i === 0 ? "left" : "right" }}>
                {h}
              </Text>
            ))}
          </View>
          ...
          <FlatList
            ...
            contentContainerStyle={{ paddingBottom: 120 }}
            ListEmptyComponent={
              <Text style={{ color: colors.tabBarInactive, textAlign: "center", marginTop: 20 }}>No expenses found</Text>
            }
          />
        </>
      )}
